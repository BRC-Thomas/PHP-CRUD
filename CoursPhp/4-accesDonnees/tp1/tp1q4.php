...
$ok = filter_input(INPUT_POST, 'ok', FILTER_DEFAULT);
...
if ($ok) {
    try {
        ...
        $filtre = [
            'immat' => FILTER_DEFAULT,
            'modele' => FILTER_DEFAULT,
  ...
  <table>
    ...
    <?php
        ...
        foreach ($voitures as $ligne):
            ?>
    <tr>
      <td><input type="text" name="<?= $ligne['immat'] ?>[immat]" value="<?= $ligne['immat'] ?>"></td>
      <td><?php choix($modeles, 'modele', $ligne['id_modele'],
                        $ligne['immat']) ?></td>
      <td><?php choix($couleurs, 'couleur', $ligne['couleur'],
                        $ligne['immat']) ?></td>
      <td><input type="date" name="<?= $ligne['immat'] ?>[date]" value="<?= $ligne['datevoiture'] ?>"></td>
    </tr>
    <?php endforeach; ?>
  </table>
